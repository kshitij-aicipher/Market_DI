...
def squeeze_df_for_tfidf(x):
    # Must be here for joblib unpickling
    if hasattr(x, "to_numpy"):
        x = x.to_numpy()
    return x.ravel()

def prepare_df_for_inference(df: pd.DataFrame) -> pd.DataFrame:
    # Standard cleaning pipeline
    df = clean_prices(df, ["actual_price", "discounted_price"])
    df = clean_percent(df, "discount_percentage")
    text_cols = ["about_product", "review_title", "review_content"]
    df = build_text_all(df, text_cols, "text_all")
    return df
